...
#include <morpher/util/tinyxml2.h>
...
bool DFG::parseXML(std::string fileName)
{
	...
	printf("------------ XML parser :: ITER -------------\n");
	//parse iteration variables
	XMLElement *iter = NULL;
	output_ss << "xmlParse :: iteration variables...\n";
	iter = doc.FirstChildElement("ITER");
	if(&this->stride)
		iter->QueryIntAttribute("stride", &this->stride);
	if(&this->max_count)
		iter->QueryIntAttribute("max_count", &this->max_count);
	printf("------------ XML parser :: DFG -------------\n");
	XMLElement *DFG = doc.FirstChildElement("DFG");
	int node_count;
	printf("------------ XML parser :: Query count -------------\n");
	DFG->QueryIntAttribute("count", &node_count);
	//	std::map<int,DFGNode*> parsedNodes;
	printf("------------ XML parser :: Parse Nodes -------------\n");
	//parse all nodes
	...
	for (int i = 0; i < node_count; ++i)
	{
		...
		int shiftVal;
		if (node->QueryIntAttribute("SHIFT", &shiftVal) == XML_SUCCESS)
		{
			currDFGNode.shiftVal = shiftVal;
			currDFGNode.hasShift = true;
		}

		int offsetVal;
		if (node->QueryIntAttribute("OFFSET", &offsetVal) == XML_SUCCESS)
		{
			currDFGNode.offsetVal = offsetVal;
			currDFGNode.hasOffset = true;
		}
		int memVal;
		if (node->QueryIntAttribute("M", &memVal) == XML_SUCCESS)
		{
			currDFGNode.isMemRelated = true;
		}
	...
	printf("------------ XML parser :: Parse Connections -------------\n");
	node = NULL;
	for (int i = 0; i < node_count; ++i)
	{
		if (node == NULL)
		...
		while (input)
		{
			...
			//std::cout << "THILINI :: idx" << idx << " input_idx=" << input_idx << "\n";

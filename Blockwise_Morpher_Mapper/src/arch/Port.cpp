...
int CGRAXMLCompile::Port::getLat()
{
	CGRA *cgra = this->getMod()->getCGRA();
	int ii = cgra->get_t_max();
	//printf("NEW II=%d PE_T=%d latency=%d\n",int(ceil(ii/this->getMod()->getCGRA()->vec_size)),this->getMod()->getPE()->T,latency);
	//assert(latency % int(ceil(ii/this->getMod()->getCGRA()->vec_size)) == this->getMod()->getPE()->T);
	return latency;
}

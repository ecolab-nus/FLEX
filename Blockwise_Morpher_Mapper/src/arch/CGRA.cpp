...
Module *CGRAXMLCompile::CGRA::ParseModule(json &module_desc, Module *parent, string module_name, string type, int t, int x, int y)
{
	...
	if(type == "MDP")
	{
		...
		ret->Name2Port["I3"] = ret->getInPort("I3");
		ret->Name2Port["I4"] = ret->getInPort("I4");
...
void ExpandPattern(json &input, json &connections, json &submods)
{
	...
	std::pair<string, string> submod_grid_iter;
	string mod_type = input["MODS_ITER"]["MOD"];
	string mod_name = input["MODS_ITER"]["name"];
	json mod_iter;
	mod_iter["name"] = mod_name;

	mod_iter["X"] = 0;
	mod_iter["Y"] = 0;
	submods[mod_type].push_back(mod_iter);
	submod_grid_iter = make_pair(mod_type,mod_name);
		...
		for (int y = 0; y < ydim; y++)
		{
			for (int x = 0; x < xdim; x++)
			{
					...
					(dest_y >= 0) && (dest_y < ydim))
				{
					...
					//printf("[THILINI] making connections for old ::%s %s\n",conn_next_str.c_str(),conn_from_str.c_str());
	...
	for (auto &el : input["CONNECTIONS_ITER"])
	{
		string curr_port_str = el["FROM_PORT"];
		string to_port_str = el["TO_PORT"];
		string tox_str = el["TO_X"];
		...
		if (tox_str.compare("X") == 0)
		{
			toy = atoi(toy_str.c_str());
			...
			for (int x = 0; x < xdim; x++)
			{
				...
				int dest_y = toy;
					...
					if ((dest_x >= 0) && (dest_x < xdim) &&
						(dest_y >= 0) && (dest_y < ydim))
					{
						string conn_next_str = submod_grid[dest_x][dest_y].second + "." + to_port_str;
						string conn_from_str = submod_grid_iter.second + "." + curr_port_str;
						printf("[THILINI] making connections ::%s %s\n",conn_next_str.c_str(),conn_from_str.c_str());
						connections[conn_from_str].push_back(conn_next_str);
					}
			}
		}

		if (toy_str.compare("Y") == 0)
		{
			tox = atoi(tox_str.c_str());
...
unordered_set<PE *> CGRAXMLCompile::CGRA::getAllPEList()
{
	...
	if (!subModArr.empty())
	{
		//printf("[THILINI] :: From submod array...\n");

...
void CGRAXMLCompile::FU::createFU(int numberDPs)
{
	...
	for (int i = 0; i < numberDPs; ++i)
	{
		...
		portName_I2.clear();
		std::string portName_I3;
		portName_I3.clear();
		std::string portName_I4;
		portName_I4.clear();

		portName_P += "DP" + std::to_string(i) + "_P";
		portName_I1 += "DP" + std::to_string(i) + "_I1";
		portName_I2 += "DP" + std::to_string(i) + "_I2";
		portName_I3 += "DP" + std::to_string(i) + "_I3";
		portName_I4 += "DP" + std::to_string(i) + "_I4";
		...
		inputPorts.push_back(new Port(portName_I3, IN, this));
		inputPorts.push_back(new Port(portName_I4, IN, this));
		//N2NFIX
	...
	for (int i = 0; i < numberDPs; ++i)
	{
		...
		insertConnection(getInPort(portName_I3), newDP->getInPort("I3"));
		insertConnection(getInPort(portName_I4), newDP->getInPort("I4"));
		...
		if (DataPath *dp1 = dynamic_cast<DataPath *>(submod))
		{
			...
			for (Module *submod : this->subModules)
			{
				if (DataPath *dp2 = dynamic_cast<DataPath *>(submod))
				{
					...
					insertConnection(dp1->getOutPort("T"), dp2->getInPort("I3"));
					insertConnection(dp1->getOutPort("T"), dp2->getInPort("I4"));
				}
			}
		}
	}

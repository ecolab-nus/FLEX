...
std::vector<LatPort> Module::getNextPorts(LatPort currPort, HeuristicMapper *hm)
{
	...
	//std::cout << "CurrPort=" << currP->getFullName() << "\n";
	...
	for (Port *p : connectedTo[currP])
	{
		...
		if (!conflicted)
		{
			...
			if((p->getName().find("ITREG_RI") != std::string::npos) && (currP->getName().find("ITREG_RO") != std::string::npos))
			{
						std::cout << p->getFullName() << "  " << currP->getName() << "\n";
					    std::cout << "tdiff = " << p->getPE()->getCGRA()->get_t_max() << " lat=" << lat << "\n";
						tdiff = p->getPE()->getCGRA()->get_t_max();
			}
...
std::pair<Port *, Port *> Module::getRegPort(std::string Pname)
{
	...
	//std::cout << "Inserting reg port = " << Pname << "\n";

	for (std::pair<Port *, Port *> portpair : regPorts)
	{
		if (portpair.first->getName() == ri)
		{
			//std::cout << "Inserting reg port r1= " << portpair.first->getName() << "\n";
			return portpair;
		}

		if (portpair.first->getName() == Pname)
		{
			//std::cout << "Inserting reg port first= " << portpair.first->getName() << "\n";
			return portpair;
		}

		if (portpair.second->getName() == Pname)
		{
			//std::cout << "Inserting reg port second= " << portpair.second->getName() << "\n";
			return portpair;
		}
	}
	assert(false);
...
void Module::insertConnection(Port *src, Port *dest)
{
	...
	//std::cout << "Connections = " << src->getName() << " -> " << dest->getName() << "\n";
}
...
{
	//printf("Getting Inports\n");
	for (Port *p : inputPorts)
	{
		//printf("Input Ports->%s Pname->%s\n",p->getFullName().c_str(),Pname.c_str());

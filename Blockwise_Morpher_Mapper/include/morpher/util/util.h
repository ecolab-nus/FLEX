#include <morpher/util/argparse.h>
...
struct arguments
{
	...
	bool use_json = false;
	int ndps = 1;
	int maxiter = 2;  // for PathFinderMapper,
...
inline arguments parse_arguments(int argn, char *argc[]){
	...
	.add_option<int>("-r", "--max_iter", "max iteration", 15)
	...
	.add_option<int>("", "--max_II", "max II", 100)
...
#ifdef FLEX
	.add_option<int>("-v", "--vecSize", "vector size", 1)
#endif
        .parse(argn, argc);
...
#ifdef FLEX
	ret.vec_size = args.get_option<int>("-v");
#endif
